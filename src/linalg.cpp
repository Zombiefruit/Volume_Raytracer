...

{
  ...
  out.rows[0] = vec4( v.x,   0,   0, 0 );
  out.rows[1] = vec4(   0, v.y,   0, 0 );
  out.rows[2] = vec4(   0,   0, v.z, 0 );
  out.rows[3] = vec4(   0,   0,   0, 1 );
...

{
  ...
  float s = 1 / tan( (fovy_in_degrees/180.0*M_PI) / 2.0 );
...

{
  ...
  vec3 z = (eye-centre).normalize();
  vec3 x = (up ^ z).normalize();
  vec3 y = (z ^ x).normalize();

  out.rows[0] = vec4( x.x, x.y, x.z, x * (-1*eye) );
  out.rows[1] = vec4( y.x, y.y, y.z, y * (-1*eye) );
  out.rows[2] = vec4( z.x, z.y, z.z, z * (-1*eye) );
  out.rows[3] = vec4(   0,   0,   0,            1 );
